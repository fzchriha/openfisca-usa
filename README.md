...
These elements are described in different folders. All the modelling happens within the `openfisca_usa` folder.
...
```sh
git clone https://github.com/openfisca/openfisca-usa.git
cd openfisca-usa
...
Substitute your package's country name for `openfisca_usa` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_usa/situation_examples/couple.json \
